...
function create(data, useCard) {
	...
	const f3Card = f3Chart.setCard(f3.CardHtml)
		.setCardDisplay([["First Name", "Middle Name"],["Last Name"]])
		.setCardDim({})
		.setMiniTree(false)
	...
	data.forEach(d => {
		...
		all_select_options.push({label: `${d.data["Full Name"]}`, value: d["id"]})
	})
	...
	if (!isMobile() && useCard) {
		const f3EditTree = f3Chart.editTree()
			.fixed(true)
			.setFields(["Full Name", "Other Name(s)", "Profession(s)", "Birth", "Death", "Age"])
